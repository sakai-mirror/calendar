...
public abstract class RecurrenceRuleBase implements RecurrenceRule
{
	...
	*/
	...
	/**
	 * Construct with time limit and specific days of the week
	 * @param recurrence an instance of an iCal4j recurrance object
	 */
	...
	*/
	public List generateInstances(TimeRange prototype, TimeRange range, TimeZone timeZone)
	{
		...
		if (this.iCalRecurrence != null) {
			DateList dateList = this.iCalRecurrence.getDates(new net.fortuna.ical4j.model.DateTime(prototype.firstTime().getTime()), new net.fortuna.ical4j.model.Date(range.firstTime().getTime()), new net.fortuna.ical4j.model.Date(range.lastTime().getTime()), Value.DATE_TIME);
			int sequenceNum = 1;
			for (java.util.Iterator i = dateList.iterator(); i.hasNext();) {
				net.fortuna.ical4j.model.Date aDate = (net.fortuna.ical4j.model.Date)i.next();
				rv.add(new RecurrenceInstance(TimeService.newTimeRange(aDate.getTime(), prototype.duration()), sequenceNum));
				sequenceNum++;
			}
			return rv;
		}
